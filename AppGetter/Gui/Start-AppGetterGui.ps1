...
.DESCRIPTION
    WPF-based GUI for packaging from a download URL or a local installer,
...
function Show-FolderBrowser {
    ...
    # points at another drive, ShowDialog can throw / tear down the WPF host.
    # Root at MyComputer so any path is valid — same as Wingetter.
...
function Start-AppGetterBackgroundPackaging {
    ...
    $null = $powershell.AddScript({
        param($ModulePath, $AppName, $WebsiteUrl, $DownloadUrl, $InstallerPath, $Version, $Publisher, $OutputPath, $IconPath, $Queue)
        Import-Module $ModulePath -Force
        $onProgress = {
            param($Event)
        ...
        if ($WebsiteUrl) { $params.WebsiteUrl = $WebsiteUrl }
        if ($DownloadUrl) { $params.DownloadUrl = $DownloadUrl }
        if ($InstallerPath) { $params.InstallerPath = $InstallerPath }
        if ($Version) { $params.Version = $Version }
        if ($Publisher) { $params.Publisher = $Publisher }
        if ($IconPath) { $params.IconPath = $IconPath }
        Invoke-AppGetterPackaging @params
    }).AddArgument($modulePath).
      AddArgument($PackArguments.AppName).
      AddArgument($PackArguments.WebsiteUrl).
      AddArgument($PackArguments.DownloadUrl).
      AddArgument($PackArguments.InstallerPath).
      AddArgument($PackArguments.Version).
      AddArgument($PackArguments.Publisher).
...
$sourceFileRadio = $window.FindName('SourceFileRadio')
$sourcePathBox = $window.FindName('SourcePathBox')
...
$stepMap = @{
    1 = 0; 2 = 1; 3 = 2; 4 = 2; 5 = 3; 6 = 4; 7 = 4; 8 = 5; 9 = 6; 10 = 7; 11 = 8; 12 = 9; 13 = 10
}

$settings = Get-AppGetterSettings
$script:baseOutputPath = Get-AppGetterBaseOutputPath -Path $settings.OutputPath -PackageId (Get-PackageIdFromAppName -AppName $settings.LastAppName)
$outputPathBox.Text = $script:baseOutputPath
$appNameBox.Text = $settings.LastAppName

if ($settings.LastInstallerPath) {
    $sourceFileRadio.IsChecked = $true
    $sourcePathBox.Text = $settings.LastInstallerPath
} elseif ($settings.LastDownloadUrl) {
    $sourceUrlRadio.IsChecked = $true
    $sourcePathBox.Text = $settings.LastDownloadUrl
}

Initialize-StepList -ListControl $stepList

function Get-CurrentPackageId {
    $name = $appNameBox.Text.Trim()
    if ([string]::IsNullOrWhiteSpace($name)) {
        return $null
    }
    return Get-PackageIdFromAppName -AppName $name
}
...
function Update-SelectedSourceDisplay {
    $source = $sourcePathBox.Text.Trim()
    $useFile = [bool]$sourceFileRadio.IsChecked

    if ([string]::IsNullOrWhiteSpace($source)) {
        $selectedSourceText.Text = 'Provide a download URL or browse to an installer on this computer.'
    ...
    if ($useFile) {
        if (Test-Path -LiteralPath $source) {
            $selectedSourceText.Text = "Selected local installer: $source"
            $selectedSourceText.Foreground = ConvertTo-WpfBrush '#1B2A41'
        } else {
            $selectedSourceText.Text = "Local installer path does not exist yet: $source"
            $selectedSourceText.Foreground = ConvertTo-WpfBrush '#C62828'
        }
    } else {
        $selectedSourceText.Text = "Selected download URL: $source"
        $selectedSourceText.Foreground = ConvertTo-WpfBrush '#1B2A41'
    }
}
...
function Update-PrereqStatusDisplay {
    ...
    if ($Prerequisites.Issues.Count -eq 0) {
        $readyBits = @('Ready')
        if ($Prerequisites.WingetVersion) {
            $readyBits += "Winget $($Prerequisites.WingetVersion)"
        }
        $readyBits += "Content Prep Tool: $($Prerequisites.ContentPrepToolPath)"
        $prereqText.Text = ($readyBits -join ' | ')
...
function Set-PackControlsEnabled {
    ...
    $sourceFileRadio.IsEnabled = $Enabled
...
function Start-AppGetterPackagingFromUi {
    ...
    if ([string]::IsNullOrWhiteSpace($source)) {
        [System.Windows.MessageBox]::Show($window, 'Provide a download URL or a local installer path.', 'AppGetter', 'OK', 'Warning') | Out-Null
        return
    }
    ...
    if ($useFile -and -not (Test-Path -LiteralPath $source)) {
        [System.Windows.MessageBox]::Show($window, "Local installer not found:`n$source", 'AppGetter', 'OK', 'Warning') | Out-Null
        return
    }

    if (-not $useFile -and $source -notmatch '^https?://') {
        [System.Windows.MessageBox]::Show($window, 'Download URL must start with http:// or https://.', 'AppGetter', 'OK', 'Warning') | Out-Null
        return
    }
    ...
    Save-AppGetterSettings -OutputPath $script:baseOutputPath -LastAppName $appName -LastPackageId $packageId

    $downloadUrl = $null
    $installerPath = $null
    if ($useFile) {
        $installerPath = $source
        Add-LogLine -LogControl $logText -Message "Starting packaging for $appName from local installer..."
    } else {
        $downloadUrl = $source
        Add-LogLine -LogControl $logText -Message "Starting packaging for $appName from download URL..."
    }
...
$browseInstallerButton.Add_Click({
    try {
        $path = Show-OpenFileDialog `
            -Filter 'Windows installers (*.exe;*.msi;*.msix;*.appx)|*.exe;*.msi;*.msix;*.appx|All files (*.*)|*.*' `
            -OwnerWindow $window
        if ($path) {
            $sourceFileRadio.IsChecked = $true
...
$browseOutputButton.Add_Click({
    try {
        ...
        if ($path) {
            ...
            Update-OutputPathForSelectedApp
            $lastId = Get-CurrentPackageId
            Save-AppGetterSettings -OutputPath $script:baseOutputPath -LastAppName $appNameBox.Text.Trim() -LastPackageId $lastId
