...
function Show-AppGetterStartupError {
    ...
    try {
        ...
        [System.Windows.MessageBox]::Show($Message, 'AppGetter', [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error) | Out-Null
    } catch {
        Write-Error $Message
    }
}
...
"@
    ...
    foreach ($attempt in $attempts) {
        ...
        if ($process.WaitForExit(4000)) {
            Write-AppGetterLaunchLog -LogPath $LogPath -Message ("childExitedEarly exitCode={0} attempt={1}" -f $process.ExitCode, $attempt.Name)
            continue
        }

        Write-AppGetterLaunchLog -LogPath $LogPath -Message ("childRunning attempt={0}" -f $attempt.Name)
        return $process
    }
