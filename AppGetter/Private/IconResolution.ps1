function Get-AppGetterWebUserAgent {
    return 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 AppGetter/1.0'
}
...
function Initialize-AppGetterWebClient {
    if ([Net.ServicePointManager]::SecurityProtocol -band [Net.SecurityProtocolType]::Tls12 -eq 0) {
        [Net.ServicePointManager]::SecurityProtocol = [Net.ServicePointManager]::SecurityProtocol -bor [Net.SecurityProtocolType]::Tls12
    }

    $client = New-Object System.Net.WebClient
    $client.Headers.Add('User-Agent', (Get-AppGetterWebUserAgent))
    return $client
}

function Get-AppGetterWebString {
    param([string]$Url)

    try {
        $client = Initialize-AppGetterWebClient
...
function Get-AppGetterDomainFromUrl {
    param([string]$Url)
...
function Save-PackageIconFromUrl {
    ...
    try {
        Write-AppGetterLog -Message "Trying icon URL: $Url" -Level Info -OnProgress $OnProgress
        ...
        if (Save-BytesAsPngIcon -Bytes $bytes -OutputPath $OutputPath) {
            if (Test-SavedIconQuality -OutputPath $OutputPath) {
                Write-AppGetterLog -Message "Downloaded icon from $Url" -Level Success -OnProgress $OnProgress
                return $true
            }

            Write-AppGetterLog -Message "Rejected low-quality icon from $Url" -Level Warning -OnProgress $OnProgress
        }
    } catch {
        Write-AppGetterLog -Message "Icon URL failed ($Url): $($_.Exception.Message)" -Level Warning -OnProgress $OnProgress
    }
...
function Add-AppGetterIconCandidates {
    param(
...
function Get-IconUrlsFromHomepage {
    ...
    try {
        $html = Get-AppGetterWebString -Url $Homepage
...
function Get-IconUrlsFromWikimedia {
    ...
    foreach ($query in ($queries | Select-Object -Unique)) {
        try {
            ...
            $jsonText = Get-AppGetterWebString -Url $apiUrl
...
function Get-IconUrlsFromClearbit {
    ...
    foreach ($site in @($Homepage, $PublisherUrl)) {
        $domain = Get-AppGetterDomainFromUrl -Url $site
...
function Get-IconUrlsFromWebSearch {
    ...
    foreach ($term in ($searchTerms | Select-Object -Unique)) {
        try {
            ...
            $html = Get-AppGetterWebString -Url $searchUrl
    ...
    try {
        ...
        if ($associated) {
            ...
            if ((Test-Path $OutputPath) -and (Get-Item $OutputPath).Length -gt 0) {
                Write-AppGetterLog -Message 'Extracted associated icon from installer executable' -Level Success -OnProgress $OnProgress
                return $true
            }
        }
    } catch {
...
"@ -ErrorAction SilentlyContinue
        ...
        if ([AppGetterIconExtractor]::ExtractToPng($ExePath, $OutputPath)) {
            if (Test-Path $OutputPath -and (Get-Item $OutputPath).Length -gt 0) {
                Write-AppGetterLog -Message 'Extracted shell icon from installer executable' -Level Success -OnProgress $OnProgress
                return $true
            }
        }
    } catch {
        Write-AppGetterLog -Message "Icon extraction failed: $_" -Level Warning -OnProgress $OnProgress
    }
...
function Get-PackageIconCandidateUrls {
    ...
    Add-AppGetterIconCandidates -Candidates $candidateList -Urls (Get-KnownPackageIconUrls -PackageId $PackageId -Publisher $Publisher -Homepage $Homepage) `
        -Source 'Known' -DisplayName $DisplayName -PackageId $PackageId -Homepage $Homepage
    Add-AppGetterIconCandidates -Candidates $candidateList -Urls (Get-IconUrlsFromWikimedia -DisplayName $DisplayName -PackageId $PackageId) `
        -Source 'Wikimedia' -DisplayName $DisplayName -PackageId $PackageId -Homepage $Homepage
    Add-AppGetterIconCandidates -Candidates $candidateList -Urls (Get-IconUrlsFromClearbit -Homepage $Homepage) `
        -Source 'Clearbit' -DisplayName $DisplayName -PackageId $PackageId -Homepage $Homepage
    Add-AppGetterIconCandidates -Candidates $candidateList -Urls (Get-IconUrlsFromWingetShow -PackageId $PackageId -Version $Version) `
        -Source 'WingetShow' -DisplayName $DisplayName -PackageId $PackageId -Homepage $Homepage
    Add-AppGetterIconCandidates -Candidates $candidateList -Urls (Get-IconUrlsFromWingetManifest -PackageId $PackageId -Version $Version) `
        -Source 'WingetManifest' -DisplayName $DisplayName -PackageId $PackageId -Homepage $Homepage
    Add-AppGetterIconCandidates -Candidates $candidateList -Urls (Get-IconUrlsFromWebSearch -DisplayName $DisplayName -PackageId $PackageId -Publisher $Publisher) `
        -Source 'WebSearch' -DisplayName $DisplayName -PackageId $PackageId -Homepage $Homepage

    if ($Homepage) {
        Add-AppGetterIconCandidates -Candidates $candidateList -Urls (Get-IconUrlsFromHomepage -Homepage $Homepage) `
            -Source 'Homepage' -DisplayName $DisplayName -PackageId $PackageId -Homepage $Homepage
        Add-AppGetterIconCandidates -Candidates $candidateList -Urls (Get-SiteRootIconUrls -Url $Homepage) `
            -Source 'Favicon' -DisplayName $DisplayName -PackageId $PackageId -Homepage $Homepage
    }

    Add-AppGetterIconCandidates -Candidates $candidateList -Urls (Get-HeuristicIconUrls -PackageId $PackageId -DisplayName $DisplayName -Publisher $Publisher -Homepage $Homepage) `
    ...
    Write-AppGetterLog -Message "Resolved $($sortedCandidates.Count) icon candidate URL(s) for $PackageId" -Level Info -OnProgress $OnProgress
...
function Resolve-PackageIconCandidates {
    ...
    if (-not $StagingDirectory) {
        $StagingDirectory = Join-Path $env:TEMP ("appgetter-icon-candidates-{0}" -f ([Guid]::NewGuid().ToString('N')))
    }
    ...
    foreach ($candidate in $sortedCandidates) {
        ...
        Write-AppGetterLog -Message "Icon candidate [$($candidate.Source) score=$($candidate.Score)]: $($candidate.Url)" -Level Info -OnProgress $OnProgress
    ...
    if ($results.Count -eq 0) {
        Write-AppGetterLog -Message "No icon candidates could be downloaded for $PackageId" -Level Warning -OnProgress $OnProgress
    } else {
        Write-AppGetterLog -Message "Downloaded $($results.Count) icon candidate(s) for $PackageId" -Level Success -OnProgress $OnProgress
    }
...
function Resolve-PackageIcon {
    ...
    Write-AppGetterLog -Message "No icon could be resolved for $PackageId" -Level Warning -OnProgress $OnProgress
    return $false
}
