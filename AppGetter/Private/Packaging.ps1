function Invoke-AppGetterPackaging {
    ...
    try {
        ...
        Write-AppGetterProgress -Step 6 -TotalSteps $totalSteps -StepName 'Discovering silent install switches' -Percent 38 `
            -Message $installerFile.Name -OnProgress $OnProgress

        $switchDiscovery = $null
        if ([string]::IsNullOrWhiteSpace($InstallCommand)) {
            $switchDiscovery = Resolve-InstallerInstallCommand -InstallerPath $installerFile.FullName `
                -InstallerFileName $installerFileName -InstallerExtension $installerExtension `
                -InstallSwitchesInfo $details.InstallSwitchesInfo -SupportUrl $SupportUrl
            $installerInstallCommand = $switchDiscovery.RecommendedCommand
        } else {
            $installerInstallCommand = $InstallCommand
        }

        Write-AppGetterProgress -Step 7 -TotalSteps $totalSteps -StepName 'Generating install.ps1' -Percent 45 -OnProgress $OnProgress
