function Invoke-AppGetterPackaging {
    ...
    try {
        ...
        $details = Get-WebPackageDetails -AppName $AppName -WebsiteUrl $WebsiteUrl -DownloadUrl $DownloadUrl `
            -Version $Version -Publisher $Publisher -DeveloperUrl $DeveloperUrl -SupportUrl $SupportUrl -InstallCommand $InstallCommand
        ...
        $null = Start-WebInstallerDownload -DownloadUrl $details.DownloadUrl -DownloadDirectory $versionDirectory -FileName $installerFileName -OnProgress $OnProgress
        ...
        if ($installerFile.Extension -in '.zip', '.7z') {
            throw 'Archive installers require manual extraction. Provide -InstallCommand after extracting the installer.'
        }

        $installerInstallCommand = Get-InstallerInstallCommand -InstallerFileName $installerFile.Name `
            -InstallerExtension $installerFile.Extension -CustomInstallCommand $details.InstallCommand

        Write-AppGetterProgress -Step 4 -TotalSteps $totalSteps -StepName 'Calculating hash' -Percent 35 -Message $installerFile.Name -OnProgress $OnProgress
        $installerHash = (Get-FileHash -Path $installerFile.FullName -Algorithm SHA256).Hash

        Write-AppGetterProgress -Step 5 -TotalSteps $totalSteps -StepName 'Generating install.ps1' -Percent 45 -OnProgress $OnProgress
        ...
        Write-AppGetterProgress -Step 6 -TotalSteps $totalSteps -StepName 'Generating detection.ps1' -Percent 55 -OnProgress $OnProgress
        ...
        Write-AppGetterProgress -Step 7 -TotalSteps $totalSteps -StepName 'Generating uninstall.ps1' -Percent 65 -OnProgress $OnProgress
        $uninstallScript = New-AppGetterUninstallScript -PackageId $details.PackageId -DisplayName $details.DisplayName

        Write-AppGetterProgress -Step 8 -TotalSteps $totalSteps -StepName 'Resolving icon' -Percent 72 -OnProgress $OnProgress
        ...
        $homepageForIcon = if ($details.DeveloperUrl) { $details.DeveloperUrl } elseif ($details.WebsiteUrl) { $details.WebsiteUrl } else { $details.Homepage }
        ...
        if ($IconPath -and (Test-Path $IconPath)) {
            ...
            $iconCandidates = @(Resolve-PackageIconCandidates -PackageId $details.PackageId -DisplayName $details.DisplayName `
                -Publisher $details.Publisher -Homepage $homepageForIcon -Version $details.Version `
        ...
        Write-AppGetterProgress -Step 9 -TotalSteps $totalSteps -StepName 'Writing metadata' -Percent 80 -OnProgress $OnProgress
        ...
        Write-AppGetterProgress -Step 10 -TotalSteps $totalSteps -StepName 'Packaging .intunewin' -Percent 88 -OnProgress $OnProgress
        ...
        Save-AppGetterSettings -OutputPath $OutputPath -LastAppName $details.DisplayName `
            -LastWebsiteUrl $details.WebsiteUrl -LastDownloadUrl $details.DownloadUrl
