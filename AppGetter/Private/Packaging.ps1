    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($InstallCommand)) {
            ...
            $switchDiscoveryResult = Resolve-InstallerInstallCommand -InstallerPath $installerFile.FullName `
                -InstallSwitchesInfo $details.InstallSwitchesInfo -AppName $AppName -OnProgress $OnProgress
            $installerInstallCommand = $switchDiscoveryResult.InstallCommand
        } else {
            $installerInstallCommand = $InstallCommand
            Write-AppGetterProgress -Step 6 -TotalSteps $totalSteps -StepName 'Using custom install command' -Percent 40 `
                -Message 'InstallCommand parameter provided' -OnProgress $OnProgress
        }

        Write-AppGetterProgress -Step 7 -TotalSteps $totalSteps -StepName 'Generating install.ps1' -Percent 50 -OnProgress $OnProgress
        ...
        Write-AppGetterProgress -Step 9 -TotalSteps $totalSteps -StepName 'Generating uninstall.ps1' -Percent 66 -OnProgress $OnProgress
        $uninstallScript = New-AppGetterUninstallScript -PackageId $details.PackageId -DisplayName $details.DisplayName

        Write-AppGetterProgress -Step 10 -TotalSteps $totalSteps -StepName 'Resolving icon' -Percent 74 -OnProgress $OnProgress
        ...
        Write-AppGetterProgress -Step 11 -TotalSteps $totalSteps -StepName 'Writing metadata' -Percent 82 -OnProgress $OnProgress
        ...
        return [PSCustomObject]@{
            ...
            Metadata             = $metadata
            Details              = $details
            SwitchDiscovery      = $switchDiscoveryResult
        }
    }
    catch {
