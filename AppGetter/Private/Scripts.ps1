...
function New-AppGetterReadmeMarkdown {
    ...
    $licensing = $PackageDetails.Licensing
    $licensingPattern = if ($licensing -and $licensing.Pattern) { $licensing.Pattern } else { 'Unknown' }
    $licensingSource = if ($licensing -and $licensing.Source) { $licensing.Source } else { 'Unknown' }
    $licensingNotes = if ($licensing -and $licensing.Notes) { $licensing.Notes } else { 'Not provided' }
...
| **Licensing pattern** | $licensingPattern |
| **Licensing source** | $licensingSource |
| **Licensing notes** | $licensingNotes |
...
"@
    ...
    $appJson = @{
        ...
        installerContext     = 2
        architecture         = 2
        licensingPattern     = if ($PackageDetails.Licensing) { $PackageDetails.Licensing.Pattern } else { 'Unknown' }
        licensingSource      = if ($PackageDetails.Licensing) { $PackageDetails.Licensing.Source } else { 'Unknown' }
        licensingNotes       = if ($PackageDetails.Licensing) { $PackageDetails.Licensing.Notes } else { '' }
        licensingConfidence  = if ($PackageDetails.Licensing) { [int]$PackageDetails.Licensing.ConfidenceScore } else { 0 }
        licensing            = if ($PackageDetails.Licensing) {
            @{
                pattern         = $PackageDetails.Licensing.Pattern
                source          = $PackageDetails.Licensing.Source
                notes           = $PackageDetails.Licensing.Notes
                confidenceScore = [int]$PackageDetails.Licensing.ConfidenceScore
                detectionNote   = $PackageDetails.Licensing.DetectionNote
                evidenceSummary = @($PackageDetails.Licensing.EvidenceSummary)
                sourceUrls      = @($PackageDetails.Licensing.SourceUrls)
            }
        } else {
            $null
        }
    ...
    $win32Notes = "Generated by AppGetter at $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [Web|$($PackageDetails.PackageId)]"
    if ($PackageDetails.Licensing -and $PackageDetails.Licensing.Pattern) {
        $win32Notes = "$win32Notes [License|$($PackageDetails.Licensing.Pattern):$($PackageDetails.Licensing.Source)]"
    }
