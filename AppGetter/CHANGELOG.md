...
- **Extensionless download URLs**: marketing links like `.../download-the-v-one-software` are normalized to `.msi`/`.exe` from file signatures so Sandbox can find them, and `install.ps1` references are updated
